...
void Default_Play_Mode_Read_Audio_Buttons(void)
{
    for (int i = 0; i < 16; i++)
    {
        ...
        else if (Audio_Display_Read_Button(i) == 0 && Audio_Is_Slot_Free(i) == 0)
        {
            if (Audio_Processor_Is_Clip_Queued(i) && Audio_Get_Clip(i)->is_repeating == 0 && Audio_Get_Clip(i)->play_through == 0)
            {
                Audio_Processor_Remove_Clip(i);
            }
        }
